	 ...
	 */
     function goStartMode(){
    	 ...
    	 if(sSrc.indexOf("btn_confirm2.png") < 0 ){
    		 welBtnConfirm.attr("src","/assets/editor/popup/btn_confirm2.png");
     ...
     function goReadyMode(){
    	 ...
    	 if(sSrc.indexOf("btn_confirm2.png") >= 0 ){
	    	 ...
	    	 welBtnConfirm.attr("src","/assets/editor/popup/btn_confirm.png");
    	 }
     }   
    ...
    function addImage(ofile){
		...
		if( !!bExceedLimitTotalSize ){
	        ...
	        aFileList.push('	<a onclick="delImage(\'img'+nImageInfoCnt+'\')"><img class="del_button" src="/assets/editor/popup/btn_del.png"  width="14" height="13" alt="첨부 사진 삭제"></a>');
			aFileList.push('	</li> ');   
    ...
    function html5Upload() {	
    	...
    	sUploadURL= '/editor_photos/create'; 	//upload URL
    ...
    function callAjaxForHTML5 (tempFile, sUploadURL){
		...
		oAjax.header("name",'editor_photo[photo]');		
	...
	function onAjaxError (){
 	...
 	function callFileUploader (){
 		oFileUploader = new jindo.FileUploader(jindo.$("uploadInputBox"),{
 			sUrl  : '/editor_photos/create.text',	//샘플 URL입니다.
 	        sCallback : location.href.replace(/\/[^\/]*$/, '') + '/editor/callback',	//업로드 이후에 iframe이 redirect될 콜백페이지의 주소
...
// 	    		oCustomEvent.stop(); 수행시 bAllowed 가 false이더라도 alert이 수행되지 않음
	 ...
	 */
 	function setPhotoToEditor(oFileInfo){
 		oFileInfo[0].sFileName=oFileInfo[0].sFileName.replace(/%2F/g,"/");
 		oFileInfo[0].sFileURL=oFileInfo[0].sFileURL.replace(/%2F/g,"/");
